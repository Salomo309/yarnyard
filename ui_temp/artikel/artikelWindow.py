...
"}")
        ...
        # self.btn_back.clicked.connect(self.kembali)
        self.btn_prev_inside.clicked.connect(self.prev)
        self.btn_next_inside.clicked.connect(self.next)
    ...
    def do_change_page(self):
        ...
        if button == self.btn_next_inside:
            if current_index == self.stackedWidget.count() - 1:
                self.stackedWidget.setCurrentIndex(0)
            else:
                self.stackedWidget.setCurrentIndex(current_index - 1)
        elif button == self.btn_prev_inside:
            if current_index == 0:
                self.stackedWidget.setCurrentIndex(self.stackedWidget.count() - 1)
            else:
                self.stackedWidget.setCurrentIndex(current_index - 1)
    ...
    def change_background_img(self, index):
        if index == 0:
            self.text_description_artikel.setPlainText("Ini Raflesia")
            self.widget_img.setStyleSheet("#widget_img {background-image: url(ui_temp/artikel/rafflesia.png); background-attachment: fixed;}")
        elif index == 1:
            self.text_description_artikel.setPlainText("Ini Kaktus")
            self.widget_img.setStyleSheet("#widget_img {background-image: url(ui_temp/artikel/kaktus.png); background-attachment: fixed;}")
